...
export default function Services({ user }: any) {
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      if (isEditing) res = await api.put(`/api/companies`, values);
      else res = await api.post(`/api/companies`, values);
  ...
  const handleDelete = async (item: any) => {
    try {
      appContext.onOpenLoading();
      const { data } = await api.delete(`/api/companies?_id=${item._id}`);
  ...
  return (
    <Page
      user={user}
      path='/companies'
      title='NF Freelancer - Cadastro de empresas'

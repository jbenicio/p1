...
export default function Services({ user }: any) {
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      if (isEditing) res = await api.put(`/api/nfs`, values);
      else res = await api.post(`/api/nfs`, values);
  ...
  const getData = async () => {
    try {
      appContext.onOpenLoading();
      const { data } = await api.get(`/api/nfs`);
  ...
  const handleDelete = async (item: any) => {
    try {
      appContext.onOpenLoading();
      const { data } = await api.delete(`/api/nfs?_id=${item._id}`);
  ...
  return (
    <Page
      user={user}
      path='/nfs'
      title='NF Freelancer - Cadastro de NF'
    ...
    >
      ...
      >
        <DrawerOverlay />
        <DrawerContent>
          <DrawerHeader borderBottomWidth='1px'>Notas Fiscais</DrawerHeader>
          <DrawerBody>
            <HStack>
              <FormControl
                mb={3}
            ...
            <HStack>
              ...
              >
                ...
                <Input
                  type='date'
                  name='date'
                  value={moment(formik.values.date, 'YYYY-MM-DD').format(
                    'YYYY-MM-DD'
                  )}
                  onChange={formik.handleChange}
                />
              ...
              <FormControl
                ...
                isRequired
                isInvalid={
                  !moment(formik.values.payDate, 'YYYY-MM-DD').isValid()
                }
              >
                <FormLabel fontSize={{ base: 'sm', md: 'md', lg: 'md' }}>
                  Data da cobrança
                ...
                <Input
                  type='date'
                  name='payDate'
                  value={moment(formik.values.payDate, 'YYYY-MM-DD').format(
                    'YYYY-MM-DD'
                  )}
                  onChange={formik.handleChange}
                />
